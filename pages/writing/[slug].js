...
const ArticleFooter = ({ frontmatter, slug }) => {
  ...
  return (
    <footer className={`${pStyle} article-footer`}>
      ...
      <p>
        This piece is part of a small writing shelf for process notes, research reflections, and
        design decisions that do not need a full case study.
      </p>
...
const Article = ({ frontmatter, content, readingTime, slug }) => {
  const canonicalUrl = frontmatter.canonicalUrl || `https://harritaito.com/writing/${slug}`;

  return (
    <div className="Article">
      <Head>
        <title>{`${frontmatter.title} | Harri Halonen`}</title>
        <meta name="description" content={frontmatter.summary || ""} key="description" />
        <link rel="canonical" href={canonicalUrl} key="canonical" />
        <meta property="og:title" content={`${frontmatter.title} | Harri Halonen`} key="og-title" />
        <meta property="og:description" content={frontmatter.summary || ""} key="og-description" />
